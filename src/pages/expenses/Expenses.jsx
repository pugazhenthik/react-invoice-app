...
const columns = [
  { column: 'id', title: '#' },
  { column: 'expense_date', title: 'Expense Date' },
  { column: 'reference_no', title: 'Reference No.' },
  { column: 'customers.display_name', title: 'Customer' },
            ...
            <tbody>
              {data?.data.map((expense, index) => (
                ...
                >
                  ...
                  <td className="p-3">{expense.reference_no}</td>
                  <td className="p-3">{expense.customer.name}</td>
                  <td className="p-3">{expense.amount}</td>
                  <td className="p-3 w-44 flex">
                    <EditButton model={model} resourceId={expense.id} />
                    <DeleteButton onDelete={onDelete} resourceId={expense.id} />
                  </td>
                </tr>
              ))}
            </tbody>
